...
class ObservationService {
  ...
  Future<List<Map<String, dynamic>>> searchInsectsByName(String query) async {
    try {
      ...
      final response = await http.get(
        Uri.parse('$baseUrl/api/insects/search?query=$query&locale=es&page=1&per_page=10'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
  ...
  Future<Observation?> createObservation({
    ...
    try {
      ...
      final Map<String, dynamic> observationData = {
        ...
        'observation_date': observationDate.toIso8601String().split('T')[0], // Solo la fecha
        'latitude': latitude,
        'longitude': longitude,
        ...
        'photos': photoUrls.map((url) => {'photo_url': url}).toList(),
      };
